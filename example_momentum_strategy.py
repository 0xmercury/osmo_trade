...
from src import create_osmo_wallet
from src import check_dir
from decimal import Decimal
from dotenv import load_dotenv
from src.pools import Coin, Decimal, SwapAmountInRoute
...
class NewStrategyInstance:
            ...
            try:
                ...
                if len(historical_price_data_queue) < _total_block_data:
                    ...
                    if old_block_height != new_block_height:
                        ...
                        historical_price_data_queue.append({'bid':bid_ask_data.bid,'ask':bid_ask_data.ask, 'block_heigh':block_height})
    ...
    def run_strategy(self, datafeed: DataFeed = None):
        ...
        historicl_old_block_point = 3               # here you can define how many old block data ypu want to keep as a history
        old_block_height = 0                        # store always last block height 
        new_block_height = 0                        # store always latest block height 
        is_long_position = False                    # Flag to check whether we have taken a position or not
        ...
        print("\nhistorical_price_data_queue", historical_price_data_queue, len(historical_price_data_queue) )
        ...
        while True:
            ...
            try:
                    ...
                    if old_block_height != new_block_height:
                        ...
                        print("\n\n*************** New Block Start*************\n\n")
                        print("Old Height :{}, New Height: {}".format(old_block_height, new_block_height))
                        ...
                        if bid_ask_data.ask > historical_price_data_queue[0]['ask'] and bid_ask_data.ask > historical_price_data_queue[1]['ask'] and  bid_ask_data.ask > historical_price_data_queue[2]['ask'] and not is_long_position:
                            ...
                            routes = [SwapAmountInRoute(pool_id= self.pool_id, denom= pool_assets[1])]
                            asset_balance = [item for item in all_token_balance if item.denom== pool_assets[0]][0]
                            ...
                            routes = [SwapAmountInRoute(pool_id= self.pool_id, denom= pool_assets[0])]
                            asset_balance = [item for item in all_token_balance if item.denom== pool_assets[1]][0]
                        ...
                        historical_price_data_queue.append({'bid':bid_ask_data.bid, 'ask':bid_ask_data.ask, 'block_heigh':new_block_height})
