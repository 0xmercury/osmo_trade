...
class NewStrategyInstance:
    ...
    def get_entry_exit_spreads(self, pool_reserves_dict):
        ...
        osmosis_bid_ask = datafeed._bid_ask_calculation(
            self.pool_id, self.token_0_amount, self.token_1_amount, pool_reserves_dict, reverse=False)
        entry_spread, exit_spread = self.signal_using_spread(
            binance_bid_ask, osmosis_bid_ask)
    ...
    def get_binance_price(symbol: str = "ATOMUSDT"):
        price_data = requests.get(
            "https://fapi.binance.com/fapi/v1/ticker/bookTicker?symbol={}".format(symbol)).json()
        bid_ask = BidAskPrice(
    ...
    def strategy_data(self, datafeed: DataFeed = None):
        ...
        while True:
            ipdb.set_trace()
            pool_reserves, height = datafeed.get_pool_reserves( pool_ids=[939])
            ...
            ipdb.set_trace()
            entry_spread, exit_spread = self.get_entry_exit_spreads(
                pool_reserves)
            if entry_spread > 10 and not is_long_position:
                print("Go For long Position")
                routes = [SwapAmountInRoute(pool_id=self.pool_id[0], denom=usdc_pool_assets[1]), SwapAmountInRoute(
                    pool_id=self.pool_id[1], denom=atom_pool_assets[0])]
                asset_balance = [
                    item for item in all_token_balance if item.denom == usdc_pool_assets[0]][0]
                ...
                routes = [SwapAmountInRoute(pool_id=self.pool_id[1], denom=atom_pool_assets[1]), SwapAmountInRoute(
                    pool_id=self.pool_id[0], denom=usdc_pool_assets[0])]
                asset_balance = [
                    item for item in all_token_balance if item.denom == atom_pool_assets[0]][0]
                tx_hash = tx.broadcast_exact_in_transaction(routes=routes, token_in=Coin(amount=int(
                    asset_balance.amount*0.98), denom=atom_pool_assets[0]), slippage=Decimal(0.002), pools=pool_reserves)
                print("Transaction Hash: ", tx_hash)
                is_long_position = False
...
ipdb.set_trace()
ENV_FILE_DIR = os.path.join(CURR_DIR.replace(
    "examples", ""), "envs/strategy.env")
